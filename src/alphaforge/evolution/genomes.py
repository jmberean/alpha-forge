...
from alphaforge.strategy.genome import PositionSizing, StrategyGenome
...
class ExpressionGenome:
    ...
    def to_strategy_genome(self) -> StrategyGenome:
        """Convert ExpressionTree to StrategyGenome.

        The formula is stored in parameters for later execution by the
        discovery system's tree evaluator.
        """
        return StrategyGenome(
            name=f"discovered_{self.tree.hash[:8]}",
            description=f"GP: {self.tree.formula}",
            version="1.0",
            sizing_method=PositionSizing.FIXED,
